...
async function animateResolution(move,token){for(const step of move.steps){if(token!==runToken)return;const clearing=step.groups.flatMap(group=>group.cells.map(([row,col])=>({row,col,color:group.color})));game.board=step.beforeBoard;game.effect={clearing,progress:0};chainCall(step.chain,step.count);if(!await tween(760,token,p=>game.effect.progress=p))return;game.board=step.poppedBoard;game.effect={burst:burstParticles(clearing),progress:0};game.score+=step.score;game.maxChain=Math.max(game.maxChain,step.chain);updateUI();if(!await tween(480,token,p=>game.effect.progress=p))return;const gravity=fallingCells(step.poppedBoard,step.afterBoard);game.board=gravity.base;if(gravity.falling.length){game.effect={falling:gravity.falling,progress:0};if(!await tween(420,token,p=>game.effect.progress=1-(1-p)**3))return;}game.board=step.afterBoard;game.effect=null;updateUI();await sleep(300)}game.board=move.resultBoard;game.effect=null;updateUI();await sleep(200);}
