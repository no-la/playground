...
function draw(){
  ...
  if(game.effect?.clearing){const p=game.effect.progress;for(const cell of game.effect.clearing){const x=cell.col*W,y=(cell.row-1)*H,cx=x+W/2,cy=y+H/2;ctx.save();ctx.globalAlpha=Math.max(.12,1-p*.85);ctx.strokeStyle=p<.55?"#fff":PALETTE[cell.color];ctx.lineWidth=3+8*p;ctx.shadowColor=PALETTE[cell.color];ctx.shadowBlur=18+28*p;ctx.beginPath();ctx.arc(cx,cy,W*(.36+.3*p),0,Math.PI*2);ctx.stroke();ctx.fillStyle=`rgba(255,255,255,${Math.max(0,.7-p)})`;ctx.beginPath();ctx.arc(cx,cy,W*(.18+.34*p),0,Math.PI*2);ctx.fill();ctx.restore();}}
  if(game.effect?.falling){for(const cell of game.effect.falling){const row=cell.from+(cell.to-cell.from)*game.effect.progress;drawPuyo(ctx,cell.color,cell.col*W,(row-1)*H,W,Math.sin(game.effect.progress*Math.PI)*-.06);}}
  if(game.effect?.burst){for(const particle of game.effect.burst){const p=game.effect.progress,x=particle.x+particle.dx*p,y=particle.y+particle.dy*p+80*p*p;ctx.save();ctx.globalAlpha=1-p;ctx.fillStyle=particle.color;ctx.shadowColor=particle.color;ctx.shadowBlur=8;ctx.beginPath();ctx.arc(x,y,particle.size*(1-p*.5),0,Math.PI*2);ctx.fill();ctx.restore();}}
...
async function tween(duration,token,update){const start=performance.now();while(performance.now()-start<duration){if(token!==runToken)return false;update(Math.min(1,(performance.now()-start)/duration));draw();await sleep(16)}update(1);draw();return true;}
function fallingCells(from,to){const base=from.map(row=>[...row]),falling=[];for(let col=0;col<COLS;col++){const sources=[],targets=[];for(let row=ROWS-1;row>=0;row--){if(from[row][col]!==EMPTY)sources.push({row,color:from[row][col]});if(to[row][col]!==EMPTY)targets.push({row,color:to[row][col]});}sources.forEach((source,i)=>{const target=targets[i];if(target&&target.row!==source.row){base[source.row][col]=EMPTY;falling.push({from:source.row,to:target.row,col,color:source.color});}});}return {base,falling};}
function burstParticles(cells){return cells.flatMap(cell=>Array.from({length:7},(_,i)=>{const angle=Math.PI*2*i/7+cell.row*.7;return {x:(cell.col+.5)*W,y:(cell.row-.5)*H,dx:Math.cos(angle)*(28+Math.random()*32),dy:Math.sin(angle)*(28+Math.random()*32)-24,size:2+Math.random()*4,color:PALETTE[cell.color]};}));}
async function animateResolution(move,token){for(const step of move.steps){if(token!==runToken)return;const clearing=step.groups.flatMap(group=>group.cells.map(([row,col])=>({row,col,color:group.color})));game.board=step.beforeBoard;game.effect={clearing,progress:0};chainCall(step.chain,step.count);if(!await tween(430,token,p=>game.effect.progress=p))return;game.board=step.poppedBoard;game.effect={burst:burstParticles(clearing),progress:0};game.score+=step.score;game.maxChain=Math.max(game.maxChain,step.chain);updateUI();if(!await tween(280,token,p=>game.effect.progress=p))return;const gravity=fallingCells(step.poppedBoard,step.afterBoard);game.board=gravity.base;if(gravity.falling.length){const maxDrop=Math.max(...gravity.falling.map(cell=>cell.to-cell.from));game.effect={falling:gravity.falling,progress:0};if(!await tween(260+maxDrop*35,token,p=>game.effect.progress=1-(1-p)**3))return;}game.board=step.afterBoard;game.effect=null;updateUI();await sleep(180)}game.board=move.resultBoard;game.effect=null;updateUI();await sleep(140);}
function chainCall(n,count){const el=$("#chain-call");el.innerHTML=`<b>${n} CHAIN!</b><small>${count} PUYO POP</small>`;el.classList.remove("show");void el.offsetWidth;el.classList.add("show");}
